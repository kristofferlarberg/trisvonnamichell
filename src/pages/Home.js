...
const Home = ({ match }) => {
  ...
  const { data: timelines, isError, isLoading, isSuccess } = useQuery('create-timelines', createTimelines)
  ...
  const handleLoad = (i) => {
    if (allLoaded.length === 0) {
      timelines.results.forEach(work => {
        if (work.data.work_script.length && work.data.work_preview_image.url) {
    ...
    <>
      <Main loaded={loaded} mobile={isMobile}>
        ...
        {timelines.results.map((item, i) => {
          ...
          return (
            item.data.work_script.length > 0 && item.data.work_preview_image.url && <TimeLine
              renditions={false}
              loaded={loaded}
